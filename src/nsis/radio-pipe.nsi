...
;General
  ...
  InstallDir "$PROGRAMFILES\radio-pipe"
  
  ;Get installation folder from registry if available
  InstallDirRegKey HKCU "Software\radio-pipe" ""

  ;Request application privileges for Windows Vista
  RequestExecutionLevel admin
  
  !define REG_UNINSTALL "Software\Microsoft\Windows\CurrentVersion\Uninstall\radio-pipe"
...
Section "radio-pipe" Main
SectionIn RO
  WriteRegStr HKLM "${REG_UNINSTALL}" "DisplayName" "RadioPipe"
  ...
  File ${PROJECT_BUILD_DIR}\radio-pipe.exe
  File /nonfatal ${PROJECT_BUILD_DIR}\*.dll

  ;Store installation folder
  WriteRegStr HKCU "Software\radio-pipe" "" $INSTDIR
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\radio-pipe.exe"
  Delete "$INSTDIR\*.dll"
  RMDir /r "$INSTDIR"

  !insertmacro un.RemoveFromSystemPath
  
  DeleteRegKey /ifempty HKCU "Software\radio-pipe"
